...
class OrganisationAdminDashboardView(LoginRequiredMixin, UserPassesTestMixin, TemplateView):
    ...
    def get_context_data(self, **kwargs):
        ...
        # Scope to the user's own org (OrgAdmin) or all (PlatformAdmin/superuser)
        if user.is_superuser or user.role == 'PlatformAdmin':
            ...
            booking_qs   = Booking.objects.filter(organisation=organisation)
            resource_qs  = Resource.objects.filter(organisation=organisation)
