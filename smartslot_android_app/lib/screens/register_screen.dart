...
class _RegisterScreenState extends State<RegisterScreen> {
  ...
  Future<void> _loadOrgs() async {
    try {
      ...
      if (res.statusCode == 200) {
        ...
        setState(() => _organisations = data.map((o) => {'id': o['id'], 'name': o['name']}).toList());
      }
    } catch (_) {}
  }
  ...
  @override
  void dispose() {
    _usernameCtrl.dispose(); _emailCtrl.dispose(); _firstNameCtrl.dispose();
    _lastNameCtrl.dispose(); _passwordCtrl.dispose(); _phoneCtrl.dispose();
    super.dispose();
  }
  ...
  Future<void> _submit() async {
    ...
    if (_accountType == 'employee' && _selectedOrgId == null) {
      setState(() => _error = 'Please select your organisation'); return;
    }
    ...
    final data = {
      ...
      if (_accountType == 'employee' && _selectedOrgId != null) 'organisation_id': _selectedOrgId,
    };
      ...
      body: SingleChildScrollView(
        ...
        child: Form(
          ...
          child: Column(crossAxisAlignment: CrossAxisAlignment.stretch, children: [
            _AccountTypeSelector(selected: _accountType, onChanged: (v) => setState(() { _accountType = v; _selectedOrgId = null; })),
            ...
            Row(children: [
              Expanded(child: TextFormField(controller: _firstNameCtrl,
                  decoration: const InputDecoration(labelText: 'First Name'),
                  validator: (v) => v!.isEmpty ? 'Required' : null)),
              const SizedBox(width: 12),
              Expanded(child: TextFormField(controller: _lastNameCtrl,
              ...
              _organisations.isEmpty
                  ? Container(
                      ...
                      child: const Row(children: [
                        ...
                        SizedBox(width: 12),
                        Text('Loading organisations...', style: TextStyle(color: AppColors.textMuted)),
                      ]))
                  : DropdownButtonFormField<int>(
                      initialValue: _selectedOrgId,
                      dropdownColor: Theme.of(context).colorScheme.surface,
                      decoration: const InputDecoration(labelText: 'Select Your Organisation',
                          prefixIcon: Icon(Icons.business_outlined, color: AppColors.textMuted)),
                      items: _organisations.map((o) => DropdownMenuItem<int>(
                          value: o['id'] as int, child: Text(o['name']))).toList(),
                      onChanged: (v) => setState(() => _selectedOrgId = v),
                      validator: (_) => _accountType == 'employee' && _selectedOrgId == null ? 'Select your organisation' : null,
                    ),
            ],
            const SizedBox(height: 24),
            ElevatedButton(
...
class _AccountTypeSelector extends StatelessWidget {
  ...
  Widget build(BuildContext context) {
    return Column(crossAxisAlignment: CrossAxisAlignment.start, children: [
      ...
      Row(children: [
        Expanded(child: _TypeCard(icon: Icons.public, title: 'External User',
            subtitle: 'Browse & contact organisations', selected: selected == 'external',
            onTap: () => onChanged('external'))),
        const SizedBox(width: 12),
        Expanded(child: _TypeCard(icon: Icons.badge_outlined, title: 'Employee',
            subtitle: 'Book resources at my organisation', selected: selected == 'employee',
...
class _TypeCard extends StatelessWidget {
  final IconData icon; final String title, subtitle; final bool selected; final VoidCallback onTap;
      ...
      child: AnimatedContainer(
        ...
        child: Column(children: [
          ...
          Text(title, textAlign: TextAlign.center,
              style: TextStyle(color: selected ? AppColors.primary : Theme.of(context).textTheme.bodyLarge?.color,
